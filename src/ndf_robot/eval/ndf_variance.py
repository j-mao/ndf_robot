...
def main(args, global_dict):
    ...
    for grasp_demo in demo_target_info_list:
        demo_shape_pts_world = grasp_demo['demo_obj_pts']
        demo_query_pts_world = grasp_demo['demo_query_pts']
        demo_shape_pts_world = torch.from_numpy(demo_shape_pts_world).float().to(device)
        demo_query_pts_world = torch.from_numpy(demo_query_pts_world).float().to(device)
        ...
        demo_query_pts_cent_perturbed = demo_query_pts_cent + (torch.randn(demo_query_pts_cent.size()) * perturb_scale).to(device)
        ...
        target_latent = model.extract_latent(demo_model_input).detach()
        demo_latents_list.append(target_latent)


    query_pts = torch.randn(500, 3).to(device)
    query_pts.requires_grad_()
    opt = torch.optim.Adam([query_pts], lr=1e-2)
    pbar = tqdm(range(500))
    for i in pbar:
        feats_list = []
        query_dists = []
        for grasp_demo, latent in zip(demo_target_info_list, demo_latents_list):
            demo_shape_pts_world = grasp_demo['demo_obj_pts']
            demo_shape_pts_world = torch.from_numpy(demo_shape_pts_world).float().to(device)
            demo_shape_pts_mean = demo_shape_pts_world.mean(0)

            demo_ee_mat = util.matrix_from_pose(util.list2pose_stamped(grasp_demo['demo_ee_pose_world']))
            query_pts_hom = torch.cat((query_pts, torch.ones_like(query_pts[..., :1])), dim=-1)
            query_pts_transformed = (query_pts_hom @ torch.from_numpy(demo_ee_mat.T).float().to(device))[..., :-1]
            query_pts_transformed = query_pts_transformed - demo_shape_pts_mean

            act_hat = model.forward_latent(latent, query_pts_transformed[None, ...])
            feats_list.append(act_hat)

            demo_gripper_pts_canon = torch.from_numpy(grasp_demo['demo_gripper_pts_canon']).float().to(device)
            demo_gripper_pts_ctr = demo_gripper_pts_canon.mean(dim=-2)
            query_dists.append(torch.sqrt(torch.nn.functional.mse_loss(query_pts, demo_gripper_pts_ctr.expand(query_pts.shape), reduction="none").sum(dim=-1)).mean())

        std_loss = torch.cat(feats_list, dim=0).std(dim=0).mean()
        dis_loss = torch.stack(query_dists).mean()
        reg_loss = torch.sqrt(1e-9+torch.nn.functional.mse_loss(query_pts[None, :, :].expand(len(query_pts), -1, -1), query_pts[:, None, :].expand(-1, len(query_pts), -1), reduction="none").sum(dim=-1)).mean()
        loss = 3*std_loss + dis_loss - 0.3*reg_loss
        # loss = -std_loss - 0.01*reg_loss + 0.03 * dis_loss
        opt.zero_grad()
        loss.backward()
        opt.step()
        pbar.set_description("Query point loss: std[{:.4f}] dis[{:.4f}] reg[{:.4f}] total[{:.4f}]".format(std_loss.item(), dis_loss.item(), reg_loss.item(), loss.item()))

    for grasp_demo in demo_target_info_list:
        demo_ee_mat = util.matrix_from_pose(util.list2pose_stamped(grasp_demo['demo_ee_pose_world']))
        query_pts_hom = torch.cat((query_pts, torch.ones_like(query_pts[..., :1])), dim=-1)
        query_pts_transformed = (query_pts_hom @ torch.from_numpy(demo_ee_mat.T).float().to(device))[..., :-1]
        grasp_demo['demo_query_pts'] = query_pts_transformed.detach().cpu().numpy()

    if False:
        trimesh_util.trimesh_show([grasp_demo['demo_gripper_pts_canon'], query_pts.detach().cpu().numpy()], show=False).show(flags={"axis": True})

    # XXX: BEGIN CRAZY PLOT

    if False:
        mean = grasp_demo['demo_gripper_pts_canon'].mean(axis=0) - np.array([0, 0.25, 0.25])
        canvas = np.zeros((100, 100, 3))
        query_pts_hom = torch.tensor([
            [mean[0], mean[1]+i/200., mean[2]+j/200., 1.]
            for i in range(100)
            for j in range(100)
        ]).float().to(device)
        feats_list = []
        ...
        stds = torch.cat(feats_list, dim=0).std(dim=0).mean(dim=-1).detach().cpu().numpy()
        stds = (stds - stds.min()) / (stds.max() - stds.min())
        for i in range(100):
            for j in range(100):
                if stds[i*100+j] >= 0.5:
                    canvas[i, j, 0] = min(1, 4*stds[i*100+j]-2)
                if stds[i*100+j] <= 0.5:
                    canvas[i, j, 2] = min(1, 2-4*stds[i*100+j])
                canvas[i, j, 1] = min(1, 2-4*abs(stds[i*100+j]-0.5))
        for pt in grasp_demo['demo_gripper_pts_canon']:
            coord = (200 * (pt - mean)).astype(int)
            canvas[coord[1], coord[2], :] = 0
        plt.imshow(canvas)
